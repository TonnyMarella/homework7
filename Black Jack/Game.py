...
class Game:
    ...
    def launching(self):
        while True:
            bots_count = int(input('Hello, white bots count: '))
        ...
        if self.dealer.full_points > 21:
            ...
            for player in self.players:
                if player.full_points == self.dealer.full_points:
                    ...
                    print(player, 'player has', player.full_points, 'points so it eq with dealer points')

                elif player.full_points > self.dealer.full_points:
                    player.money += player.bet * 2
                    if isinstance(player, Bot):
                        print(player, 'are win!')
                    elif isinstance(player, Player):
                        print('You are win!')

                elif player.full_points < self.dealer.full_points:
                    if isinstance(player, Bot):
                        print(player, 'are lose!')
                    elif isinstance(player, Player):
                        print('You are lose!')
